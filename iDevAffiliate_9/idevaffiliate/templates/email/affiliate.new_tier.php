...
$adata->execute(array($insert_tier));
...
// ------------------------------------------------
$edata=$db->query("select aff_new_tier_sub, aff_new_tier_body from idevaff_email_$email_table_extension");
$indv_data=$edata->fetch();
$sub=$indv_data['aff_new_tier_sub'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['aff_new_tier_body'];
// ------------------------------------------------
