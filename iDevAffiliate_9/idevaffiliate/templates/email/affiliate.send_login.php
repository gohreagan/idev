...
// ------------------------------------------------
$edata=$db->query("select login_subject, login_body from idevaff_email_$email_table_extension");
$indv_data=$edata->fetch();
$sub=$indv_data['login_subject'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['login_body'];
// ------------------------------------------------
...
$con = preg_replace("/{password}/", "$new_password", $con);
...
// ----------------------------------------------------------
$get_attachment = $db->query("select template_id, filename from idevaff_email_attachments where template_id = '7' and enabled = '1'");
