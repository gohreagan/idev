...
$edata=$db->query("select admin_rec_subject, admin_rec_body from idevaff_email_english");
$indv_data=$edata->fetch();
$sub=$indv_data['admin_rec_subject'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['admin_rec_body'];
// ------------------------------------------------
...
$mail->isHTML(true);
$content = nl2br($con) . "<br /><br />Affiliate: " . $name . "<br /><br />--------<br />Message Auto-Sent By iDevAffiliate " . $version;
} else {
$mail->isHTML(false);
