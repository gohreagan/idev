...
$e=$indv_data['email'];
// ------------------------------------------------
$edata=$db->query("select admin_notify_logo_sub, admin_notify_logo_body from idevaff_email_english");
$indv_data=$edata->fetch();
$sub=$indv_data['admin_notify_logo_sub'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['admin_notify_logo_body'];
// ------------------------------------------------
...
$mail->isHTML(true);
$content = nl2br($con) . "<br /><br />--------<br />Username: " . $name . "<br />--------<br /><br />Message Auto-Sent By iDevAffiliate " . $version;
} else {
$mail->isHTML(false);
