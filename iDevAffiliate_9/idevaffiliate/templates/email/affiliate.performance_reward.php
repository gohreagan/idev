...
// ------------------------------------------------
$edata=$db->query("select rew_subject, rew_body from idevaff_email_$email_table_extension");
$indv_data=$edata->fetch();
$sub=$indv_data['rew_subject'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['rew_body'];
// ------------------------------------------------
...
// ----------------------------------------------------------
$get_attachment = $db->query("select template_id, filename from idevaff_email_attachments where template_id = '6' and enabled = '1'");
