...
$edata=$db->query("select new_debit_subject, new_debit_body from idevaff_email_$email_table_extension");
$indv_data=$edata->fetch();
$sub=$indv_data['new_debit_subject'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['new_debit_body'];
// ------------------------------------------------
...
// ------------------------------------------------
$deb_amount=number_format($deb_amount,$decimal_symbols);

if($cur_sym_location == 1) { $debit_amount = $cur_sym . $deb_amount; }
if($cur_sym_location == 2) { $debit_amount = $deb_amount . " " . $cur_sym; }
$debit_amount = $debit_amount . " " . $currency;

$get_debit_lang=$db->query("select debit_amount_label, debit_reason_label from idevaff_language_$email_table_extension");
$indi_get_debit_lang=$get_debit_lang->fetch();

$amt = $indi_get_debit_lang['debit_amount_label'] . ": " . $debit_amount;
$reason = $indi_get_debit_lang['debit_reason_label'] . ": " . $debitreason;
...
// ----------------------------------------------------------
$get_attachment = $db->query("select template_id, filename from idevaff_email_attachments where template_id = '4' and enabled = '1'");
...
$mail->isHTML(true);
$content = nl2br($con) . "<br /><br />" . $amt . "<br />" . $reason . "<br /><br />" . nl2br($signature);
} else {
$mail->isHTML(false);
$content = $con . "\n\n" . $amt . "\n" . $reason . "\n\n" . $signature;
}
