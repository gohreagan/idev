...
$edata=$db->query("select new_app_subject, new_app_body from idevaff_email_$email_table_extension");
$indv_data=$edata->fetch();
$sub=$indv_data['new_app_subject'];
$sub = preg_replace("/{company_name}/", "$sitename", $sub);
$con=$indv_data['new_app_body'];
// ------------------------------------------------
...
// ------------------------------------------------
$payoute=number_format($payoute,$decimal_symbols);

if($cur_sym_location == 1) { $commission_amount = $cur_sym . $payoute; }
if($cur_sym_location == 2) { $commission_amount = $payoute . " " . $cur_sym; }
$commission_amount = $commission_amount . " " . $currency;

if ($email == 'top') {
$amt = "Commission Amount: " . $commission_amount;
} elseif ($email == 'tier') {
$amt = "Tier Commission Amount: " . $commission_amount;
} elseif ($email == 'override') {
$amt = "Override Commission Amount: " . $commission_amount; }
// ------------------------------------------------
...
$mail->isHTML(true);
$content = nl2br($con) . "<br /><br />" . $amt . "<br /><br />" . nl2br($signature);
} else {
$mail->isHTML(false);
$content = $con . "\n\n" . $amt . "\n\n" . $signature;
}
