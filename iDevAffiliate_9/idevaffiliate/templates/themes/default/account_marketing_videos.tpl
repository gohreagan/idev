...
<ul class="list-group">
<li class="list-group-item"><label>{$mv_body_title}:</label> {$mvideos_link_results[nr].mvideos_link_name}<span class="{if $language_direction == '1' }pull-left{else}pull-right{/if}"><a href="video_preview.php?affiliate_id={$link_id}&video_id={$mvideos_link_results[nr].mvideos_link_id}" class="btn btn-success btn-sm" target="_blank">{$mv_preview_button}</a></span></li>
...
<textarea rows="8" class="form-control" style="direction: ltr !important;"><link rel="stylesheet" href="{$install_url}/templates/source/videos/css/elite.css" type="text/css" media="screen"/>
...
<textarea rows="2" class="form-control" style="direction: ltr !important;"><div id="idev_video_player"></div></textarea></li>
</ul>
</div>
</div>
</div>
