...
<title>{$sitename} - {$header_title}</title>
		...
		{if $language_direction == '1'}<link rel="stylesheet" type="text/css" href="templates/themes/{$active_theme}/css/style_login_rtl.css">
		{else}<link rel="stylesheet" type="text/css" href="templates/themes/{$active_theme}/css/style_login.css">{/if}
...
<!--<body class="fixed-left login-page" style="background:{$background_color}; background-image: url({$background_image}); no-repeat; min-height: 500px; background-attachment: fixed !important; background-size: cover !important;">-->
