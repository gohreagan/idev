...
<h1 style="color:{$heading_text};">{$change_comm_page_title}</h1>
</div>
...
{if isset($commission_updated)}<div class="alert alert-success">{$commission_updated}</div>{/if}
...
<label class="col-sm-3 control-label">{$change_comm_page_new_comm}</label>
<div class="col-sm-6">                  
...
<button type="submit" class="btn btn-inverse">{$change_comm_page_button}</button>
</div>
</div>
<br /><div class="alert alert-danger">{$change_comm_page_warning}</div>
</form>
	
{else}

<div class="alert alert-danger">{$no_styles_available}</div>
