...
<h1 style="color:{$heading_text};">{$coupon_title}</h1>
</div>
...
<div class="portlet-heading" style="background: linear-gradient(to right, rgba(0, 0, 0, 0.40) 0%, rgba(0, 0, 0, 0) 100%), {$portlet_1};"><div class="portlet-title" style="color:{$portlet_1_text};"><h4>{$cc_vanity_title}</h4></div></div>
<div class="portlet-body">

{if isset($cc_display_edit_errors)}
<div class="alert alert-danger">{$cc_error_list}</div>
{/if}
{if isset($cc_creation_success)}
<div class="alert alert-success">{$cc_creation_success}</div>
{/if}

<form class="form-horizontal" role="form" method="post" action="account.php">
<input type="hidden" name="page" value="44">
<div class="form-group">
<label class="col-sm-3 control-label">{$cc_vanity_field}</label>
<div class="col-sm-6"><input type="text" class="form-control" name="coupon_code_request" value="">
</div>
</div>
<div class="form-group">
<div class="col-sm-offset-3 col-sm-6">
<button type="submit" class="btn btn-inverse">{$cc_vanity_button}</button>
</div>
</div>
</form>
