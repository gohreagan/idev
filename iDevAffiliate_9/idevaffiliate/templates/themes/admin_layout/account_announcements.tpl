...
<script>
	...
	function post_to_wall(message, link, source) {
        //if client is using v2.9, use below code
        FB.ui({
            display: 'iframe',
		...
		/*
		 ...
		 //below code will work facebook api version < 2.9
		 FB.ui({
		 method: 'feed',
		 link: link,
		 picture: source,
...
<div class="portlet-heading" style="background: linear-gradient(to right, rgba(0, 0, 0, 0.40) 0%, rgba(0, 0, 0, 0) 100%), {$portlet_4};"><div class="portlet-title" style="color:{$portlet_4_text};"><h4>{$marketing_group_title}: {$announcement_link_results[nr].group_name}</h4></div></div>
<div class="portlet-body">
