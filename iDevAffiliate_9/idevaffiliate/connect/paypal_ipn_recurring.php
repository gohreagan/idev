...
##   -----------------------------
     $chktxn = $db->prepare("select aff_id from idevaff_pp_transactions where order_num = ?");
	 $chktxn->execute(array($idev_ordernum));
##   -----------------------------

if ($chktxn->rowCount()==0) {
...
$data=array("$idev_ordernum");
$statement = $db->prepare("update idevaff_pp_transactions set rec = '0' where order_num = ?");
...
##   -----------------------------
     $chktxn = $db->prepare("select aff_id from idevaff_pp_transactions where order_num = ?");
	 $chktxt->execute(array($idev_ordernum));
##   -----------------------------
