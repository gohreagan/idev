...
$st->execute(array(null,null,$username,$password,$approved,$payable,$tax_id,$company,$first_name,$last_name,$email,$address_1,$address_2,$city,$state,$zip,$country,$phone,$fax,$website,$paypal_account,$payout_type,$payout_level,$signup_date));
...
if (isset($tier)) {
//$newid = $db->query("select id from idevaff_affiliates where username = '$username'");
$newid = $db->prepare("select id from idevaff_affiliates where username = ?");
$newid->execute(array($username));
$getid = $newid->fetch();
$insertid = $getid['id'];
$insert_tier = $tier;
//$db->query("insert into idevaff_tiers (parent, child) VALUES ('$tier', '$insertid')");
...
// ----------------------------------------------------------------
if ($tier) {
//$db->query("delete from idevaff_tlog where ta = '$tier' and ti = '$ip_address'");
